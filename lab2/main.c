...
void child_task(int X, int balance) {
    ...
    send_all(X, STARTED, log_started_fmt, get_physical_time(), my_id, getpid(), getppid(), get_balance());
    receive_all(X, STARTED);
    printf(log_received_all_started_fmt, get_physical_time(), my_id);
    fprintf(events_log_fd, log_received_all_started_fmt, get_physical_time(), my_id);
    Message message;
    ...
    while (1) {
        ...
        if (message.s_header.s_type == TRANSFER) {
            ...
            if (id_from_msg == PARENT_ID) {
                ...
                printf(log_transfer_out_fmt, get_physical_time(), my_id, order->s_amount, order->s_dst);
                fprintf(events_log_fd, log_transfer_out_fmt, get_physical_time(), my_id, order->s_amount, order->s_dst);
            } else {
                update_balance((balance_t)(get_balance() + order->s_amount));
                ...
                printf(log_transfer_in_fmt, get_physical_time(), my_id, order->s_amount, order->s_src);
                fprintf(events_log_fd, log_transfer_in_fmt, get_physical_time(), my_id, order->s_amount, order->s_src);
            }
        } else if (message.s_header.s_type == STOP || message.s_header.s_type == DONE) {
            if (message.s_header.s_type == STOP) {
                send_all(X, DONE, log_done_fmt, get_physical_time(), my_id, get_balance());
    ...
    printf(log_received_all_done_fmt, get_physical_time(), my_id);
    fprintf(events_log_fd, log_received_all_done_fmt, get_physical_time(), my_id);
