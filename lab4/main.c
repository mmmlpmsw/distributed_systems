...
local_id my_id;
fd_pair** pipes;
CriticalSectionRequest cs_requests[MAX_PROCESS_ID + 1];
bool* processes_finished;
int cs_requests_len = 0;
int children_num;
...
void add_cs_request(local_id process_id, timestamp_t time) {
    int ptr = 0;
    while (ptr < cs_requests_len && cs_requests[ptr].time < time)
        ptr++;
    while (ptr < cs_requests_len && cs_requests[ptr].time == time && cs_requests[ptr].process_id < process_id)
        ptr++;

    cs_requests_len++;

    for (int i = cs_requests_len - 1; i > ptr; i--)
        cs_requests[i] = cs_requests[i - 1];

    cs_requests[ptr].process_id = process_id;
    cs_requests[ptr].time = time;
}

void remove_cs_request(local_id process_id) {
    int ptr = 0;
    while (cs_requests[ptr].process_id != process_id && ptr < cs_requests_len)
        ptr++;

    for (int i = ptr; i < cs_requests_len - 1; i++)
        cs_requests[i] = cs_requests[i + 1];
    cs_requests_len--;
}
...
void process_request(Message* message, local_id from) {
    if (message->s_header.s_type == CS_REQUEST) {
        add_cs_request(from, message->s_header.s_local_time);
...
int request_cs(__attribute__((unused)) const void * self) {
    ...
    add_cs_request(my_id, get_lamport_time());
    ...
    while (1) {
        ...
        if (all_replied && cs_requests[0].process_id == my_id)
            break;
    }
...
int release_cs(__attribute__((unused)) const void * self) {
    send_all(children_num, CS_RELEASE, "");
    remove_cs_request((local_id) my_id);
    return 0;
}
